...
export async function readKeycloakRealm(
    ...
    const { groups, groupMembers } = await readKeycloakGroups(client, config, logger);

    const usersByName = new Map(users.map(u => [u.metadata.name, u]));
    const groupsById = new Map(groups.map(g => [g.metadata.annotations?.[KEYCLOAK_ID_ANNOTATION]!, g]));
...
export async function readKeycloakGroups(
    ...
    for (const keycloakGroup of keycloakGroups) {
        ...
        const { addedGroups, addedGroupMembers } = await addGroup(client, keycloakGroup, undefined, config, logger);
...
export async function addGroup(
    ...
    groupMembers.set(keycloakGroup.id!, Array.from(await readGroupMembers(client, config, keycloakGroup), x => x.username!))
    ...
    if (keycloakGroup.subGroups) {
        for (const subgroup of keycloakGroup.subGroups) {
            ...
            const { addedGroups, addedGroupMembers } = await addGroup(client, subgroup, keycloakGroup, config, logger);
...
export async function readGroupMembers(
    ...
    return await client.groups.listMembers({ id: group.id!, realm: config.realm });
